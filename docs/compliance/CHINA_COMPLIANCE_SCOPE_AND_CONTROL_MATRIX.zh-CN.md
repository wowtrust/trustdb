...
| `CRY-06` | `STD-39786`：完整密钥生命周期 | v1 descriptor 已记录 KeyID、suite、算法、provider、公钥、证书链和 provider reference；registry 已记录注册、有效期、撤销和 compromise 事件。SM4 envelope、生产 provider rotation/backup/destroy 仍待 #451–#453，`Partial` | 审批生成、分发、启用、备份、恢复、销毁 | Security & Cryptography | ADR-0008、密钥台账、事件链、轮换与撤销验证 | `G3` |
| `CRY-07` | `STD-CERT`：证书链和状态可验证 | descriptor 已支持受限 leaf-first DER chain、leaf 公钥精确绑定、suite/signature profile、CA/order 检查，并明确 evidence chain 不是 trust root；CRL/OCSP 和签名时状态材料仍待 #455，`Partial` | 管理 CA、CRL/OCSP、双证书和有效期 | Security & Cryptography + External Crypto & BCOS Operators | ADR-0008、证书链/错序/错 key/错 suite 负向测试、台账 | `G3` |
