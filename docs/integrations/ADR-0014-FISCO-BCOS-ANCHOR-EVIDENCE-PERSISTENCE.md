...
1. `trustdb.fisco-bcos-attempt-journal.v1` is mutable, append-only provider
   ...
   Tree Head.
2. `trustdb.fisco-bcos-anchor-proof.v1` is the immutable result evidence copied
...
The completed `AnchorProof v1` contains:
...
Until #466 verifies transaction/receipt inclusion and #467 verifies PBFT
finality from verifier-local trust roots, this proof remains
`external_observation`. It cannot promote records to L5.
