...
| C SDK native library | `v3.6.0` | `53240138c396c10cb0e1a2b7b4d5c0cdaa0ac539` | [Official C SDK release](https://github.com/FISCO-BCOS/bcos-c-sdk/releases/tag/v3.6.0), which is the native library release named by the Go SDK README. GitHub did not expose asset digests; TrustDB downloaded and SHA-256 hashed every published asset on 2026-07-24. The Linux/amd64 library reports embedded commit `0`, while the Windows/amd64 DLL reports `73a3f93b2b88bd4af93e1df9c8f8c4f0be57a8ae`. These platform build values never replace the release provenance commit: TrustDB accepts them only together with the exact platform, filename, byte length, and SHA-256 pin. |
...
# Optional bounded overrides. Keep them identical for a comparison pair.
scripts/fisco-bcos/smoke-air.sh \
  --mode guomi --work-dir /tmp/fisco-guomi-long \
  --performance-warmup 10 --performance-samples 100
...
Every compiler-backed run now carries a dedicated `performance` block. The
...
consensus latency. Compare paired artifacts from the same otherwise-idle host;
do not compare committed historical evidence with a run from different
hardware, operating system, node load or sample settings.
