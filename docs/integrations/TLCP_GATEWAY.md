...
Every production profile must name the canonical public verifier descriptors
for every active TrustDB proof signer in `proof_key_descriptor_files`. These
are the public `.pub` descriptors generated by TrustDB and selected by the same
deployment generation as `keys.server_public` and any active registry verifier;
never mount a signer descriptor. The validator reads the real CBOR descriptors,
requires `kind=verifier` and `provider=public`, computes their normalized SPKI
fingerprints, and binds both descriptor and key hashes into the runtime
manifest. A profile cannot self-declare those hashes. Validation fails when the
inventory is empty, carries a private provider reference, changes after
preparation, or overlaps either gateway key. The optional `--forbid-*` flags
add rollout-time identities; they do not replace the authoritative descriptors.
...
| --- | --- |
| `TLCP_PROFILE_FILE` | Absolute path to the strict profile; all runtime paths, providers, binds, authoritative proof-descriptor paths, and timeouts come from this file. |
...
`timeouts.startup`. Rotation controllers must invoke
`tlcp-gateway-prepare-runtime reload`; that wrapper derives and enforces
`timeouts.reload`. The readiness executable independently derives and enforces
`timeouts.canary`. None of the three controller paths has a fixed 10-second
...
files, the dedicated readiness signing/encryption identities, authoritative
TrustDB public proof descriptors, every individually validated CRL, and the
exact runtime CRL bundle as one immutable generation. Never replace these files
individually in the active directory.
