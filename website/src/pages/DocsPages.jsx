...
export function DesktopDocsPage({ route }) {
  return (
    <DocsShell route={route}>
      ...
      <section className="doc-section"><h2>打包与下载</h2><CodeBlock>cd clients/desktop{"\n"}wails build</CodeBlock><Note tone="warn" title="自签名测试版">公开下载的 <code>1.0.0-beta</code> 客户端采用发布时临时生成的自签名证书，并随包附带公钥证书。macOS Gatekeeper 与 Windows SmartScreen 仍会显示未知开发者提示；请先核对 SHA-256。</Note><InlineLink href="/downloads">选择桌面客户端</InlineLink></section>
