import { ArrowRight, Desktop, HardDrives, Package, TerminalWindow, Wrench } from "@phosphor-icons/react";
...
function ArticleTitle({ index, title, lead, updated = "2026.07.19" }) {
  return (
    <header className="docs-title">
      <p>Docs / {index}</p>
      <h1>{title}</h1>
      <span>{lead}</span>
      <small>更新于 {updated} · 适用于 TrustDB 源码版本</small>
    </header>
  );
}
  ...
  return (
    <>
      ...
      <section className="docs-index section-shell">
        <div className="docs-index__intro" data-reveal><p>Choose a guide</p><h2>五份指南，<br />各取所需。</h2></div>
...
export function QuickStartPage({ route }) {
  return (
    <DocsShell route={route}>
      <ArticleTitle index="01" title="快速开始" lead="先完成完全本地、无需网络的 L1 → L3 证据闭环，再启动服务端。" />
      <section className="doc-section" data-reveal><h2>准备环境</h2><p>需要 Go 工具链与一个本地文件。当前仓库尚未发布预编译 CLI，快速开始从源码执行。</p><CodeBlock>git clone https://github.com/ryan-wong-coder/trustdb.git{"\n"}cd trustdb{"\n"}go test ./...</CodeBlock></section>
      <section className="doc-section"><h2>1. 生成客户端与服务端密钥</h2><p>TrustDB 使用 Ed25519。私钥只留在签名方；验证路径使用公钥。</p><CodeBlock>go run ./cmd/trustdb keygen --out .trustdb-dev --prefix client{"\n"}go run ./cmd/trustdb keygen --out .trustdb-dev --prefix server</CodeBlock></section>
      <section className="doc-section"><h2>2. 创建签名声明</h2><p>CLI 在本地计算内容哈希并生成 <code>.tdclaim</code>，原文件不会被上传到这一步之外。</p><CodeBlock>go run ./cmd/trustdb claim-file \{"\n"}  --file ./example.txt \{"\n"}  --private-key .trustdb-dev/client.key \{"\n"}  --tenant default --client local-client --key-id client-key \{"\n"}  --out .trustdb-dev/example.tdclaim</CodeBlock></section>
      <section className="doc-section"><h2>3. 本地提交到 Merkle 批次</h2><p>本地 <code>commit</code> 生成 L3 ProofBundle。服务端路径使用相同的证明语义。</p><CodeBlock>go run ./cmd/trustdb commit \{"\n"}  --claim .trustdb-dev/example.tdclaim \{"\n"}  --server-private-key .trustdb-dev/server.key \{"\n"}  --client-public-key .trustdb-dev/client.pub \{"\n"}  --out .trustdb-dev/example.tdproof</CodeBlock></section>
      <section className="doc-section"><h2>4. 独立验证</h2><p>验证器会重新计算文件哈希、签名与证明路径；不会信任文件中自报的证明等级。</p><CodeBlock>go run ./cmd/trustdb verify \{"\n"}  --file ./example.txt \{"\n"}  --proof .trustdb-dev/example.tdproof \{"\n"}  --server-public-key .trustdb-dev/server.pub \{"\n"}  --client-public-key .trustdb-dev/client.pub</CodeBlock><Note title="接下来">需要多人共享、异步批次、L4 全局日志或 L5 外部锚定时，继续阅读服务器文档；优先交换单文件 <code>.sproof</code>。</Note></section>
...
export function ServerDocsPage({ route }) {
  return (
    <DocsShell route={route}>
      ...
      <section className="doc-section"><h2>启动开发服务</h2><p><code>serve</code> 同时提供 HTTP ingest；可选 gRPC、Prometheus 指标与挂载于 <code>/admin</code> 的管理端。</p><CodeBlock>go run ./cmd/trustdb serve \{"\n"}  --config configs/development.yaml \{"\n"}  --server-private-key .trustdb-dev/server.key \{"\n"}  --client-public-key .trustdb-dev/client.pub \{"\n"}  --listen 127.0.0.1:8080</CodeBlock><CodeBlock label="health">curl http://127.0.0.1:8080/healthz</CodeBlock></section>
      ...
      <section className="doc-section"><h2>运维入口</h2><CodeBlock>go run ./cmd/trustdb doctor --config configs/production.yaml{"\n"}go run ./cmd/trustdb config validate --config configs/production.yaml{"\n"}go run ./cmd/trustdb backup create --metastore pebble --metastore-path ./proofs --out trustdb.tdbackup{"\n"}go run ./cmd/trustdb backup verify --file trustdb.tdbackup</CodeBlock><InlineLink href="https://github.com/ryan-wong-coder/trustdb/tree/main/configs">查看全部配置模板</InlineLink></section>
...
export function CliDocsPage({ route }) {
  return (
    <DocsShell route={route}>
      ...
      <section className="doc-section"><h2>命令地图</h2><div className="cli-map">{cliGroups.map(([title, commands]) => <div key={title}><strong>{title}</strong><code>{commands}</code></div>)}</div><CodeBlock>go run ./cmd/trustdb --help{"\n"}go run ./cmd/trustdb verify --help{"\n"}go run ./cmd/trustdb serve --help</CodeBlock></section>
...
export function SdkDocsPage({ route }) {
  return (
    <DocsShell route={route}>
      ...
      <section className="doc-section"><h2>安装</h2><p>当前没有稳定版本标签，<code>@latest</code> 会解析到仓库可用的最新伪版本。生产项目应固定具体提交。</p><CodeBlock>go get github.com/ryan-wong-coder/trustdb@latest</CodeBlock><Note tone="warn" title="版本状态">仓库当前没有正式 Release / SemVer tag。固定 commit 可以保证可重现，但不代表稳定 API 承诺。</Note></section>
...
export function DesktopDocsPage({ route }) {
  return (
    <DocsShell route={route}>
      ...
      <section className="doc-section"><h2>开发运行</h2><CodeBlock>cd clients/desktop{"\n"}go test ./...{"\n"}cd frontend && npm ci && npm run build{"\n"}cd ..{"\n"}wails dev</CodeBlock><p>前端开发预览在没有 Wails bridge 时只用于 UI 检查；真实签名、文件选择与本地数据库能力需要通过 Wails 原生壳运行。</p></section>
      <section className="doc-section"><h2>打包</h2><CodeBlock>cd clients/desktop{"\n"}wails build</CodeBlock><Note tone="warn" title="当前没有公共下载">仓库尚未发布签名、公证的正式客户端 Release。本机或 CI 产物不应被表述为公开发行版；下载页会在首个 Release 发布后列出各平台资源与校验值。</Note><InlineLink href="/downloads">查看下载状态</InlineLink></section>
...
export function MissingDocsPage() {
  return <PageHero eyebrow="404 / Documentation" title={<>这篇文档<br />还不存在。</>} lead="返回文档中心，选择已实现的五条路径。"><div className="page-hero__actions"><Link className="button button--solid" href="/docs">文档中心 <ArrowRight /></Link></div></PageHero>;
}
