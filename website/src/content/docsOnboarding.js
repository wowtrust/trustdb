...
const zhCN = {
  ...
  ui: {
    docs: "文档",
    home: "文档首页",
    updated: "更新于 2026.07.29",
    version: "适用于 TrustDB v2.0.1",
    duration: "预计用时",
    outcome: "完成后你将得到",
    prerequisites: "开始前准备",
    expected: "预期结果",
  ...
  quickStart: {
    title: "10 分钟快速开始",
    lead: "从空目录下载并验证当前 V2 发布包，完成本地签名、L3 Merkle 提交、独立验证和篡改负向测试。无需启动服务器或编译 TrustDB。",
    duration: "10 分钟",
    outcome: "example.tdproof 与一条重新计算得到的 L3 有效结果",
    prerequisites: ["macOS、Linux 或 Windows 终端", "macOS/Linux 的 curl、tar 与 SHA-256 工具，或 Windows PowerShell", "一个全新的练习目录；不要复用生产密钥目录"],
    downloadTitle: "1. 获取 v2.0.1",
    downloadBody: "官网教程固定到当前正式版。命令会按本机架构下载 Server/CLI 归档和 SHA256SUMS，校验成功后才解压；不要拿 v1 二进制执行当前教程。",
    downloadExpected: "SHA-256 检查通过，发布目录已解压，trustdb version 输出当前正式版、平台与架构。",
    allPlatforms: "可选：从源码构建",
    platformNoteTitle: "为什么必须固定正式版标签",
    platformNoteBody: "当前正式版使用 V2 密钥、模型、WAL、API 与 .sproof 代际，v1.0.0 不包含这些命令和格式。常规用户直接使用发布归档；只有开发或 provider-enabled 特殊构建才进入单独的源码构建页。",
    linuxPathLabel: "Linux amd64：下载、校验与解压",
    windowsPathLabel: "Windows x86-64：完整 PowerShell 教程",
    extractTitle: "2. 准备发布目录和练习文件",
    extractBody: "继续在已校验并解压的发布目录中操作，创建明确的输入文件。后续所有一次性状态都放在 .trustdb-dev 下。",
    extractExpected: "version 输出发布版本、平台与构建信息；example.txt 内容为 hello TrustDB。",
  ...
  sdk: {
    ...
    projectBody: "在 .trustdb-dev 下建立独立 Go module，并直接固定到与 CLI 同版本的 github.com/wowtrust/trustdb/v2 发布模块。教程不使用本地 replace，也不会意外解析到 v1 或浮动 main。",
    projectExpected: "go.mod 精确要求当前 github.com/wowtrust/trustdb/v2 正式版本且没有 replace；example.txt 与所需密钥仍保留在原练习位置。",
  ...
  server: {
    ...
    prerequisites: ["已完成 10 分钟快速开始，并保留练习用 .trustdb-dev/server.key 与 client.pub", "已记录并固定验证通过的发布版本、归档摘要或 OCI digest", "已决定二进制或 Docker 部署方式", "已规划持久数据、日志、密钥、key registry 与备份目录", "已规划反向代理、mTLS 或网络策略；不要把未保护的写入接口直接暴露到公网"],
    localTitle: "1. 先用相对路径完成冒烟测试",
    localBody: "下面的命令直接使用快速开始中已校验的发布二进制，并适合非 root 用户。macOS/Linux 会重新读取软件密钥口令；Windows 练习沿用快速开始生成的可丢弃 plaintext-dev-v1 密钥。所有 WAL、Pebble 和 proof 数据都留在相对路径，不会意外写入 /var/lib 或 /var/log。",
    localExpected: "healthz 成功，records 返回分页响应，metrics 返回 Prometheus 文本。",
    dockerTitle: "2. 拉取不可变正式版镜像，完成 mTLS 评估",
    dockerBody: "直接按发布证据中的 OCI digest 拉取 GHCR 多架构镜像，不在本地重建。正式版 entrypoint 会正确区分 version 等信息命令与 serve 启动，config/docker.yaml 明确采用 development 评估 profile，并继续使用 mTLS 而不是无证书 HTTP demo。启动前必须准备服务器证书、客户端 CA、专用健康检查客户端证书，并确保服务器证书含 trustdb DNS SAN。",
    dockerExpected: "按 digest 拉取的镜像 version 命令成功；完整挂载配置、TLS 与密钥口令后，容器进入 healthy，带健康检查客户端证书的 HTTPS 请求返回成功。",
    dockerBoundary: "随包 development profile 只用于安全评估，不是生产豁免。不要把它描述为生产部署，也不要为了启动而把监听器降级成明文。生产环境必须进入下一节的 production.yaml，并配置独立 audit signer、同步时间证据、业务侧 signer 与受信 key registry。",
    templateTitle: "3. 再迁移到 production.yaml",
    templateBody: "直接使用发布包内的 config/production.yaml。模板默认使用 /etc/trustdb、/var/lib/trustdb 和 /var/log/trustdb；创建目录、设置最小权限并替换密钥路径后再启动。生产 profile 还强制要求独立审计 signer 和持续刷新的 synchronized time-reference；缺失时会在开放业务端口前 fail closed。run_profile 只是可观测标签，--data-dir 也不会自动重定位 WAL、proofstore、日志和密钥。",
...
const en = {
  ...
  lang: "en",
  ui: {
    docs: "Docs", home: "Docs home", updated: "Updated 2026-07-29", version: "For TrustDB v2.0.1",
  ...
  nav: {
    groups: [
      ...
      ["Operate", [["Production deployment", "/docs/server"], ["Feature catalog", "/docs/features"], ["Backup and recovery", "/docs/backup-recovery"], ["Immutable security audit", "/docs/security-audit"], ["Online key lifecycle", "/docs/key-lifecycle"], ["Release verification and mirrors", "/docs/supply-chain"], ["Operations handbook", "/docs/operations"], ["FISCO BCOS anchoring", "/docs/fisco-bcos"], ["Troubleshooting", "/docs/troubleshooting"]]],
  ...
  index: {
    ...
    meta: "TrustDB onboarding path · v2.0.1", primary: "Start the 10-minute tutorial", secondary: "Understand the evidence model first",
  ...
  quickStart: {
    title: "10-minute quick start", lead: "From an empty directory, download and verify the current V2 release archive, then complete local signing, an L3 Merkle commit, independent verification, and a tamper-negative test. No server or TrustDB build is required.",
    duration: "10 minutes", outcome: "example.tdproof and a recomputed valid L3 result",
    prerequisites: ["A macOS, Linux, or Windows terminal", "curl, tar, and a SHA-256 tool on macOS/Linux, or Windows PowerShell", "A new practice directory; never reuse a production key directory"],
    downloadTitle: "1. Get v2.0.1", downloadBody: "The website tutorial is pinned to the current stable release. The command downloads the Server/CLI archive and SHA256SUMS for the host architecture and extracts only after verification; do not run this tutorial with a v1 binary.", downloadExpected: "The SHA-256 check passes, the release directory is extracted, and trustdb version reports the current stable release, platform, and architecture.",
    allPlatforms: "Optional source-build guide", platformNoteTitle: "Why the stable tag must be pinned", platformNoteBody: "The current stable release uses the V2 key, model, WAL, API, and .sproof generation. Normal users consume release archives; only development or a special provider-enabled build belongs on the dedicated source-build page.",
    linuxPathLabel: "Linux amd64: download, verify, and extract", windowsPathLabel: "Windows x86-64: complete PowerShell walkthrough",
    extractTitle: "2. Prepare the release directory and practice file", extractBody: "Continue inside the verified, extracted release directory and create an explicit input. All disposable state stays under .trustdb-dev.", extractExpected: "version reports the release, platform, and build metadata; example.txt contains hello TrustDB.",
  ...
  sdk: {
    ...
    projectTitle: "2. Create an isolated SDK example directory", projectBody: "Create a separate Go module under .trustdb-dev and pin the published github.com/wowtrust/trustdb/v2 module to the exact CLI release. The tutorial uses no local replace and cannot drift to v1 or main.", projectExpected: "go.mod requires the exact current github.com/wowtrust/trustdb/v2 stable release with no replace; example.txt and the required keys remain in their original practice locations.",
  ...
  server: {
    ...
    prerequisites: ["The 10-minute quick start completed, retaining the practice .trustdb-dev/server.key and client.pub", "A verified release version, archive digest, or OCI digest recorded and pinned", "A binary or Docker deployment decision", "Planned persistent data, log, key, key-registry, and backup locations", "A reverse proxy, mTLS, or network policy plan; do not expose an unprotected write endpoint to the public internet"],
    localTitle: "1. Begin with a relative-path smoke test", localBody: "Use the verified release binary from quick start as a non-root user. macOS/Linux prompts again for the software-key passphrase; the disposable Windows exercise keeps the plaintext-dev-v1 keys generated earlier. WAL, Pebble, and proof data stay in explicit relative paths instead of /var/lib or /var/log.", localExpected: "healthz succeeds, records returns a page, and metrics returns Prometheus text.",
    dockerTitle: "2. Pull the immutable stable image for an mTLS evaluation", dockerBody: "Pull the multi-architecture GHCR image by the OCI digest in the release evidence without rebuilding locally. The stable entrypoint correctly separates informational commands such as version from serve startup. config/docker.yaml explicitly uses the development evaluation profile and still requires mTLS rather than a certificate-free HTTP demo. Provide the server certificate, client CA, dedicated health-client certificate, and a trustdb DNS SAN before startup.", dockerExpected: "The digest-pinned image version command succeeds. With configuration, TLS, and key-passphrase mounts, the container becomes healthy and the certificate-authenticated HTTPS health request succeeds.",
    dockerBoundary: "The bundled development profile is for secure evaluation, not a production exception. Do not call it production or weaken the listener to plaintext. Production must proceed to production.yaml in the next section and provision a separate audit signer, synchronized time evidence, an application-side signer, and a trusted key registry.",
    templateTitle: "3. Then migrate to production.yaml", templateBody: "Use config/production.yaml directly from the release archive. Defaults target /etc/trustdb, /var/lib/trustdb, and /var/log/trustdb. Create directories, set least privilege, and replace key paths first. The production profile also requires a dedicated audit signer and a continuously refreshed synchronized time-reference; missing evidence fails closed before business listeners open. run_profile is only an observability label, and --data-dir does not relocate WAL, proofstore, logs, and keys automatically.",
