...
const zhCN = {
  ...
  ui: {
    ...
    version: "适用于 TrustDB v2.0.0-rc.1",
    duration: "预计用时",
    outcome: "完成后你将得到",
    prerequisites: "开始前准备",
    expected: "预期结果",
  ...
  quickStart: {
    ...
    downloadTitle: "1. 获取 v2.0.0-rc.1",
  ...
  server: {
    ...
    dockerTitle: "2. 拉取不可变 RC 镜像，再按 mTLS 基线启动",
    dockerBody: "直接按发布证据中的 OCI digest 拉取 GHCR 多架构镜像并运行 version，不在本地重建。发布包中的 config/docker.yaml 是生产 mTLS 基线，不是无证书 HTTP demo；启动服务前必须准备服务器证书、客户端 CA、专用健康检查客户端证书，并确保服务器证书含 trustdb DNS SAN。",
...
const en = {
  ...
  lang: "en",
  ui: {
    docs: "Docs", home: "Docs home", updated: "Updated 2026-07-26", version: "For TrustDB v2.0.0-rc.1",
  ...
  index: {
    ...
    meta: "TrustDB onboarding path · v2.0.0-rc.1", primary: "Start the 10-minute tutorial", secondary: "Understand the evidence model first",
  ...
  server: {
    ...
    dockerTitle: "2. Pull the immutable RC image, then start the mTLS baseline", dockerBody: "Pull the multi-architecture GHCR image by the OCI digest in the release evidence and run version without rebuilding locally. The release archive's config/docker.yaml is a production mTLS baseline, not a certificate-free HTTP demo. Before starting the service, provide the server certificate, client CA, dedicated health-client certificate, and a trustdb DNS SAN on the server certificate.", dockerExpected: "The digest-pinned image version command succeeds. After configuration, TLS, and the key-passphrase file are mounted, the container becomes healthy and the certificate-authenticated HTTPS health request succeeds.",
