...
const zhCN = {
  ...
  quickStart: {
    ...
    downloadBody: "官网教程固定到当前发布候选版。选择操作系统后复制命令，浅克隆 v2.0.0-rc.1 标签并记录实际 commit；不要拿 v1 二进制执行当前教程。",
    ...
    platformNoteBody: "v2.0.0-rc.1 使用 V2 密钥、模型、WAL、API 与 .sproof 代际，v1.0.0 不包含这些命令和格式。教程、CLI 与 SDK 必须来自同一标签；安装包和完整验真资料可在下载页获取。",
  ...
  sdk: {
    ...
    projectBody: "在 .trustdb-dev 下建立独立 Go module，并用 replace 指向刚才克隆的同一份 RC 源码。这样 SDK 与 CLI 必然来自同一个 commit，不会意外解析到 v1。生产项目直接固定 github.com/wowtrust/trustdb/v2@v2.0.0-rc.1；本地教程用 v2.0.0 占位并 replace 到源码目录。",
    projectExpected: "go.mod 要求 github.com/wowtrust/trustdb/v2 v2.0.0，并通过 replace 指向 ../..；example.txt 与所需密钥仍保留在原练习位置。",
  ...
  server: {
    ...
    localBody: "下面的命令直接使用快速开始构建的 v2.0.0-rc.1 二进制，并适合非 root 用户。macOS/Linux 会重新读取软件密钥口令；Windows 练习沿用快速开始生成的可丢弃 plaintext-dev-v1 密钥。所有 WAL、Pebble 和 proof 数据都留在相对路径，不会意外写入 /var/lib 或 /var/log。",
    localExpected: "healthz 成功，records 返回分页响应，metrics 返回 Prometheus 文本。",
    dockerTitle: "2. 构建 RC 镜像，再按 mTLS 基线启动",
    dockerBody: "先从同一份 v2.0.0-rc.1 源码构建镜像并运行 version，确认镜像本身可执行。当前 configs/docker.yaml 是生产 mTLS 基线，不是无证书 HTTP demo；启动服务前必须准备服务器证书、客户端 CA、专用健康检查客户端证书，并确保服务器证书含 trustdb DNS SAN。下面的第二组命令只在这些资产齐全后执行。",
...
const en = {
  ...
  quickStart: {
    ...
    downloadTitle: "1. Get v2.0.0-rc.1", downloadBody: "The website tutorial is pinned to this release candidate. Select your operating system, shallow-clone the v2.0.0-rc.1 tag, and record the exact commit; do not run this tutorial with a v1 binary.", downloadExpected: "git rev-parse prints the checked-out commit and go version satisfies the repository go.mod.",
    allPlatforms: "Complete source-build guide", platformNoteTitle: "Why the RC tag must be pinned", platformNoteBody: "v2.0.0-rc.1 uses the V2 key, model, WAL, API, and .sproof generation. v1.0.0 does not contain those commands or formats. The tutorial, CLI, and SDK must come from the same tag; packages and verification evidence remain on the downloads page.",
  ...
  sdk: {
    ...
    projectTitle: "2. Create an isolated SDK example directory", projectBody: "Create a separate Go module under .trustdb-dev and replace the TrustDB module with the same RC source checkout. CLI and SDK then come from one exact commit instead of resolving v1. Production projects should pin github.com/wowtrust/trustdb/v2@v2.0.0-rc.1 directly; the local tutorial uses v2.0.0 as the placeholder replaced by the source directory.", projectExpected: "go.mod requires github.com/wowtrust/trustdb/v2 v2.0.0 and replaces it with ../..; example.txt and the required keys remain in their original practice locations.",
  ...
  server: {
    ...
    dockerTitle: "2. Build the RC, then start the mTLS baseline", dockerBody: "Build the image from the same v2.0.0-rc.1 checkout and run version first. Current configs/docker.yaml is a production mTLS baseline, not a certificate-free HTTP demo. Before starting the service, provide the server certificate, client CA, dedicated health-client certificate, and a trustdb DNS SAN on the server certificate. Run the second command block only after those assets exist.", dockerExpected: "The image version command succeeds. After configuration, TLS, and the key-passphrase file are mounted, the container becomes healthy and the certificate-authenticated HTTPS health request succeeds.",
