...
const zhCN = {
  ...
  ui: {
    ...
    version: "适用于 TrustDB v2.0.0-rc.2",
    duration: "预计用时",
    outcome: "完成后你将得到",
    prerequisites: "开始前准备",
    expected: "预期结果",
  ...
  quickStart: {
    ...
    downloadTitle: "1. 获取 v2.0.0-rc.2",
    downloadBody: "官网教程固定到当前发布候选版。命令会按本机架构下载 Server/CLI 归档和 SHA256SUMS，校验成功后才解压；不要拿 v1 二进制执行当前教程。",
    downloadExpected: "SHA-256 检查通过，发布目录已解压，trustdb version 输出当前候选版、平台与架构。",
    allPlatforms: "可选：从源码构建",
    platformNoteTitle: "为什么必须固定 RC 标签",
    platformNoteBody: "当前候选版使用 V2 密钥、模型、WAL、API 与 .sproof 代际，v1.0.0 不包含这些命令和格式。常规用户直接使用发布归档；只有开发或 provider-enabled 特殊构建才进入单独的源码构建页。",
  ...
  sdk: {
    ...
    projectExpected: "go.mod 精确要求当前 github.com/wowtrust/trustdb/v2 候选版本且没有 replace；example.txt 与所需密钥仍保留在原练习位置。",
  ...
  server: {
    ...
    dockerTitle: "2. 拉取不可变 RC 镜像，完成 mTLS 评估",
    dockerBody: "直接按发布证据中的 OCI digest 拉取 GHCR 多架构镜像，不在本地重建。RC.2 的 entrypoint 会把有缺陷的服务配置注入 version，因此命令显式指定镜像内 trustdb 二进制；评估启动再用 TRUSTDB_RUN_PROFILE=development 覆盖随包配置。config/docker.yaml 仍采用 mTLS，不是无证书 HTTP demo；启动前必须准备服务器证书、客户端 CA、专用健康检查客户端证书，并确保服务器证书含 trustdb DNS SAN。",
    dockerExpected: "按 digest 拉取的镜像 version 命令成功；显式使用 development 评估 profile 并完整挂载配置、TLS 与密钥口令后，容器进入 healthy，带健康检查客户端证书的 HTTPS 请求返回成功。",
    dockerBoundary: "TRUSTDB_RUN_PROFILE=development 是 RC.2 随包 Docker 配置的明确评估修复，不是生产豁免。不要把它描述为生产部署，也不要为了启动而把监听器降级成明文。生产环境必须进入下一节的 production.yaml，并配置独立 audit signer、同步时间证据、业务侧 signer 与受信 key registry。",
...
const en = {
  ...
  lang: "en",
  ui: {
    docs: "Docs", home: "Docs home", updated: "Updated 2026-07-26", version: "For TrustDB v2.0.0-rc.2",
  ...
  index: {
    ...
    meta: "TrustDB onboarding path · v2.0.0-rc.2", primary: "Start the 10-minute tutorial", secondary: "Understand the evidence model first",
  ...
  quickStart: {
    ...
    downloadTitle: "1. Get the current release candidate", downloadBody: "The website tutorial is pinned to the current release candidate. The command downloads the Server/CLI archive and SHA256SUMS for the host architecture and extracts only after verification; do not run this tutorial with a v1 binary.", downloadExpected: "The SHA-256 check passes, the release directory is extracted, and trustdb version reports the current candidate, platform, and architecture.",
    allPlatforms: "Optional source-build guide", platformNoteTitle: "Why the RC must be pinned", platformNoteBody: "The current candidate uses the V2 key, model, WAL, API, and .sproof generation. Normal users consume release archives; only development or a special provider-enabled build belongs on the dedicated source-build page.",
  ...
  server: {
    ...
    dockerTitle: "2. Pull the immutable RC image for an mTLS evaluation", dockerBody: "Pull the multi-architecture GHCR image by the OCI digest in the release evidence without rebuilding locally. RC.2's entrypoint injects the invalid service config into version, so the command explicitly selects the trustdb binary; evaluation startup then overrides the bundled profile with TRUSTDB_RUN_PROFILE=development. config/docker.yaml still uses mTLS rather than a certificate-free HTTP demo. Provide the server certificate, client CA, dedicated health-client certificate, and a trustdb DNS SAN before startup.", dockerExpected: "The digest-pinned image version command succeeds. With the explicit development evaluation profile plus configuration, TLS, and key-passphrase mounts, the container becomes healthy and the certificate-authenticated HTTPS health request succeeds.",
    dockerBoundary: "TRUSTDB_RUN_PROFILE=development is an explicit RC.2 evaluation workaround, not a production exception. Do not call this a production deployment or weaken the listener to plaintext. Production must proceed to production.yaml in the next section and provision a separate audit signer, synchronized time evidence, an application-side signer, and a trusted key registry.",
