...
const releaseMessagesEnglish = {
  "在线密钥生命周期补丁版：受鉴权、受 RBAC 约束且完整审计的注册、查询和撤销直接作用于当前进程的追加式注册表。": "Online key-lifecycle patch: authenticated, RBAC-governed, fully audited registration, inspection, and revocation mutate the running process's append-only registry.",
  ...
  "这是与 v2.0.0 数据格式兼容的补丁版，新增在线客户端密钥控制面；V2/V5 仍不读取 v1 存储、备份、API 请求或证据文件。": "This patch is data-format compatible with v2.0.0 and adds the online client-key control plane. V2/V5 still does not read v1 storage, backups, API requests, or evidence files.",
  ...
  "直接使用发布产物": "Use published artifacts directly",
  "验证来源": "Verify provenance",
  "核对文件": "Check the files",
  "固定版本": "Pin the version",
  ...
  "2.0.1 的在线密钥生命周期 API 沿用 Admin RBAC、会话/mTLS/OIDC 与不可变安全审计，并直接更新当前进程用于 claim admission 的追加式注册表；多副本部署仍需受控分发同一有序注册表事件流。": "The 2.0.1 online key-lifecycle API reuses Admin RBAC, session/mTLS/OIDC authentication, and immutable security audit, and directly updates the append-only registry used by the running claim-admission path. Multi-replica deployments still require controlled distribution of one ordered registry event stream.",
  "在线密钥生命周期": "Online key lifecycle",
