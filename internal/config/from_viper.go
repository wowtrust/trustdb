...
func FromViper(v *viper.Viper) Config {
	...
	return Config{
		...
		Audit: Audit{
			...
			Required:                v.GetBool("audit.required"),
			Path:                    v.GetString("audit.path"),
			CheckpointPath:          v.GetString("audit.checkpoint_path"),
			SigningKey:              v.GetString("audit.signing_key"),
			MaxBytes:                v.GetInt64("audit.max_bytes"),
			Retention:               v.GetString("audit.retention"),
			TimeReferencePath:       v.GetString("audit.time_reference_path"),
			TimeMaxSampleAge:        v.GetString("audit.time_max_sample_age"),
			TimeMaxDrift:            v.GetString("audit.time_max_drift"),
			RequireSynchronizedTime: v.GetBool("audit.require_synchronized_time"),
		},
		Keys: Keys{
		...
		Admin: Admin{
			...
			PolicyPath:       v.GetString("admin.policy_path"),
			...
			LoginMaxFailures: v.GetInt("admin.login_max_failures"),
			LoginLockout:     v.GetString("admin.login_lockout"),
			CLIEnforce:       v.GetBool("admin.cli_enforce"),
			OIDCGatewayPins:  splitCSV(strings.Join(v.GetStringSlice("admin.oidc_gateway_spki_sha256"), ",")),
		},
	}
}
