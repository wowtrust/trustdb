...
func VerifyExportJSONL(ctx context.Context, input io.Reader, publicKey trustcrypto.PublicKeyDescriptor) (Stats, error) {
	...
	for scanner.Scan() {
		...
		switch line.Kind {
		case "manifest":
			if manifestSeen || lineNumber != 1 || line.CryptoSuite != publicKey.Suite || line.PublicKey == nil || !samePublicKey(*line.PublicKey, publicKey) {
				return Stats{}, errors.New("securityaudit: invalid export manifest")
			}
			manifestSeen = true
		case "event":
			if !manifestSeen || checkpointSeen || line.Event == nil {
		...
		case "checkpoint":
			if !manifestSeen || checkpointSeen || line.Checkpoint == nil {
