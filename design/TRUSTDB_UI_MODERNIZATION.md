...
- 浏览器评审数据只在显式设置 `VITE_TRUSTDB_DEMO=1` 时启用；原生运行时设备名取自真实 `client_id`，记录、证明等级、服务 RTT 与运行指标均来自现有 Wails store/API。

### Web 管理端

- 顶部导航可切换指标、记录、批次、全局树和系统设置。
- “刷新数据”并行读取真实 metrics、batch roots 和 record indexes，并每 15 秒自动刷新。
- 待提交、待锚定与接口错误由真实指标计算；接口失败明确显示“部分数据不可用”，不会回退为虚假的健康状态。
- 演示模式不请求未启动的本地后端，静态样例不会进入生产运行路径。
